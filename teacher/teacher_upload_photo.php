...
if (isset($_GET['teacherCode'])) {
    ...
    while ($rTeacher = mysql_fetch_array($q_selectTeacher)) {
        $getTeacherId = $rTeacher[0];
        $getTeacherCode = $rTeacher[1];
        $getPhoto = $rTeacher[7];
    }
    $_SESSION['teacherId'] = $getTeacherId;
}
...
if (isset($_POST['btnFinish'])) {
    ...
    if (isset($_FILES["photo"])) {
        $target_dir = "../asset/images/teacher/";
        $target_file = $target_dir . basename($_FILES["photo"]["name"]);
        $imageFileType = pathinfo($target_file, PATHINFO_EXTENSION);

        if (move_uploaded_file($_FILES["photo"]["tmp_name"], $target_file)) {
            $photo = basename($_FILES["photo"]["name"]);
            $sql_upload_img = "update teacher set photo = '$photo' where teacherCode = '$teacherCode'";
            $q_upload_img = mysql_query($sql_upload_img, $connection);
            if (!$q_upload_img) {
                echo die(mysql_error());
            }else{
                header("Location: teacher-dashboard.php?teacherCode='$teacherCode'");
    ...
    <div class="row">
        <div class="col-md-offset-1 col-md-10 form-login">
            <form role="form" method="post" action="" enctype="multipart/form-data">
                ...
                <?php
                echo "<img src='../asset/images/teacher/$getPhoto' class='user-photo' id='img_url'>";
                ?>
                <hr>
                <div class="row">
                    <input type="hidden" class="filestyle" name="teacherCode" value="<?php echo $getTeacherCode; ?>">
